...
function join(channel) {
	...
	if (document.domain == config.domain) {
		...
		ws = new WebSocket('wss://'+config.domain+'/chatws');
	}
	else {
		// for local installs
		ws = new WebSocket('ws://' + document.domain + ':'+config.port);
	}
...
function parseMedia(){
	...
	tv.onclick = function() {
		for (var i = 0; i < links.length; i++) {
			if (parse_yturl(links[i]) != null) {
				handleViewer("https://www.youtube.com/embed/"+parse_yturl(links[i])+"?autoplay=1&origin="+document.domain);
				break;
			}
		}
	};
